...
return real_count; // fallback: count exceeds largest bucket
}
#endif
...
#endif 
    // REPLACEMENT BLOCK — volume hiding via bucket padding
int total_matches = 0;
for(int writer_id: writer_subset) {
...
#ifdef ENABLE_VOLUME_HIDING
    // Compute padded size: each writer sends next_bucket(count) entries
    int padded_total = 0;
    int padded_counts[writer_subset.size()];
    for(int i = 0; i < (int)writer_subset.size(); ++i) {
        padded_counts[i] = next_bucket(output[writer_subset[i]][0]);
        padded_total += padded_counts[i];
    }
    for(int i = 0; i < (int)writer_subset.size(); ++i)
        cout << "Writer " << writer_subset[i]+1 
             << ": real=" << output[writer_subset[i]][0] 
             << " padded=" << padded_counts[i] << endl;

    // Message: for each writer, send padded_count then padded_count file IDs
    zmq::message_t search_output((padded_total + writer_subset.size()) * sizeof(int));
    uint8_t *search_output_data = (uint8_t*)search_output.data();

    for(int i = 0; i < (int)writer_subset.size(); ++i) {
        int writer_id    = writer_subset[i];
        int real_count   = output[writer_id][0];
        int padded_count = padded_counts[i];

        // Send padded count (not real count)
        memcpy(search_output_data, &padded_count, sizeof(int));
        search_output_data += sizeof(int);

        // Send real file IDs
        if(real_count > 0) {
            memcpy(search_output_data, output[writer_id] + 1, real_count * sizeof(int));
            search_output_data += real_count * sizeof(int);
        }

        // Fill remaining slots with dummy sentinel
        int dummy = DUMMY_FILE_ID;
        for(int d = real_count; d < padded_count; ++d) {
            memcpy(search_output_data, &dummy, sizeof(int));
            search_output_data += sizeof(int);
        }
    }
